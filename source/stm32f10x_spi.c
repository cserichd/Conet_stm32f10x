...
void spiInit(volatile stm32f10x_spi_t* spi, dataframe_t dff, lsbfirst_t lsb) {
     ...
     spi->CR1 |=  (STM32F10X_SPI_CR1_BR & STM32F10X_SPI_BR_PCLK_DIV256);        // select fpclk/128 for clock
     ...
     spi->CR1 &= ~(STM32F10X_SPI_CR1_BIDIMODE);
     spi->CR1 &= ~(STM32F10X_SPI_CR1_RXONLY);
     ...
     spi->CR2 |= STM32F10X_SPI_CR2_SSOE;
...
unsigned char spiReceive(volatile stm32f10x_spi_t* spi) {
     while(!(spi->SR & STM32F10X_SPI_SR_RXNE));                     // wait for RX buffer to be ready
     return spi->DR;
}
